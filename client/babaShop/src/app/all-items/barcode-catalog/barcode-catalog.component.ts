...
export class BarcodeCatalogComponent implements OnInit {
  quickAddItems: any[] = [];
  isLoading = true;
  ...
  generateEan13Svg(rawCode: string): SafeHtml {
    ...
    if (code.length !== 13) {
      return this.sanitizer.bypassSecurityTrustHtml(
        '<div style="font-size:10px;color:#999;text-align:center">Invalid</div>'
      );
    }
    ...
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w.toFixed(2)} ${h}" preserveAspectRatio="none" style="width:100%;max-width:150px;height:auto;">${rects}</svg>`;
